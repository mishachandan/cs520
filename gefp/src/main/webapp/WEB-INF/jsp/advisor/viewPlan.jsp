...
request.setAttribute("title Student", "CS520 ADVISOR");
                    ...
                    <div class="curved container">
						<h4> Golden Eagle Flight Plan <input type="button" value="Logout" style="float:right;"  onclick="location.href='${tomcatDocUrl}j_spring_security_logout'"/>  <!-- logout.html --> </h4>                     
						   <h3 style="float:center">
							...
							<table border =1 width='100%' id="languages" >
								<tr>
									<th></th>
								<c:forEach var="runway" items="${student.officialPlan.runwayList}" varStatus="colIndex" >	
									<th id="col${runway.runwayId}"><%-- <th id="col${colIndex.index}"> --%><center> ${ runway.desc }</center></th>
								</c:forEach>
								</tr>
								
								<c:forEach var="stage1" items="${student.officialPlan.stageList }" varStatus="rowIndex"><!-- stage loop -->
									<tr id="row${stage1.stageId}"> <%-- <tr id="row${rowIndex.index}"> --%> 
										<td> ${stage1.desc }
											
										</td>
									<c:forEach var="runway1" items="${student.officialPlan.runwayList }" varStatus="colIndex" ><!--  runway loop -->
											
											<%-- <c:set var="newindex" value="${colIndex.index+1 }" />
											 --%>
											
										<td id="row${stage1.stageId}col${runway1.runwayId}">	<%-- <td id="row${rowIndex.index}col${colIndex.index}">	 --%>
										<ul >
